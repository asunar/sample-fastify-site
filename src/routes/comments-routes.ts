...
import {
  ...
  CommentsInsertResponseSchema,
  CommentsUpdateSchema,
  CommentsUpdateParamsSchema,
  CommentsRowSchema,
} from "../db/refinements/comments.ts";
...
export default function commentsRoutes(db: DatabaseSync) {
  const plugin: FastifyPluginAsyncZod = async function (fastify) {
    fastify.get("/comments", {
      schema: {
        ...
        response: { 200: z.array(CommentsRowSchema) },
      },
    }, async (request) => {
      const { limit, skip } = request.query;
      // node:sqlite returns untyped rows; the serializer validates them against
      // CommentsRowSchema at runtime, so the assertion states what it enforces.
      return db.prepare("SELECT * FROM comments LIMIT ? OFFSET ?").all(limit, skip) as z.infer<typeof CommentsRowSchema>[];
    });

    fastify.get("/comments/:id", {
      schema: {
        params: CommentsUpdateParamsSchema,
        response: { 200: CommentsRowSchema, 404: ErrorSchema },
      },
    }, async (request, reply) => {
      const { id } = request.params;

      const row = db.prepare("SELECT * FROM comments WHERE id = ?").get(id) as
        z.infer<typeof CommentsRowSchema> | undefined;
      if (!row) {
        reply.code(404);
        return { statusCode: 404, error: "Not Found", message: `Comments ${id} not found` };
      }
    ...
    fastify.post("/comments", {
      schema: {
        body: CommentsInsertSchema,
        response: { 201: CommentsInsertResponseSchema, 409: ErrorSchema },
      },
    }, async (request, reply) => {
      ...
      try {
        result = db
          .prepare(`INSERT INTO comments (${columns.join(", ")}) VALUES (${placeholders})`)
          .run(...values);
      } catch (err) {
    ...
    fastify.patch("/comments/:id", {
      ...
      const existing = db.prepare("SELECT 1 FROM comments WHERE id = ?").get(id);
      if (!existing) {
        reply.code(404);
        return { statusCode: 404, error: "Not Found", message: `Comments ${id} not found` };
      }
      ...
      if (fields.length > 0) {
        values.push(id);
        try {
          db.prepare(`UPDATE comments SET ${fields.join(", ")} WHERE id = ?`).run(...values);
        } catch (err) {
          if (isUniqueViolation(err)) {
            reply.code(409);
            return { statusCode: 409, error: "Conflict", message: "Already exists" };
